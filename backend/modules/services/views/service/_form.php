	...
	<?= $form->errorSummary($model); ?>
		...
		<div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
			...
			if (!$model->isNewRecord) {
				$staffs = StaffInfo::find()->where(['status' => 1, 'designation' => $model->staff_type])->all();
			} else {
				$staffs = [];
			}
			?>
			<?= $form->field($model, 'day_staff')->dropDownList(ArrayHelper::map($staffs, 'id', 'staff_name'), ['class' => 'form-control staff-change', 'prompt' => '--select staff--']) ?>
			...
			?>
			<?= $form->field($model, 'night_staff')->dropDownList(ArrayHelper::map($staffs, 'id', 'staff_name'), ['class' => 'form-control staff-change', 'prompt' => '--select staff--']) ?>
		...
		<?php } ?>
                <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
			<?= $form->field($model, 'status')->dropDownList(['1' => 'Opened', '0' => 'Cloed']) ?>
